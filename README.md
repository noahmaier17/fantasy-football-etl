...
- Many regular season draft boards have information about injured players. This API returns a column called `status_injury`, but the data is all empty. It would be nice to be able to determine if a player is out for the rest of the season, for instance, but that would require either a better API or much more complicated work. 