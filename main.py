...
from helper_functions import stats_directory_name, add_duplicates, Player, write_to_csv, duplicate_player
...
def main():
    ...
    if directory_path.is_dir() and csv_file_path.is_file() and not ignore_cache:
        print(Fore.YELLOW + str(season_year) + " stats are already cached locally, so skipping downloading them...")
    else:
        print(Fore.CYAN + "Fetching " + str(season_year) + " Sleeper PPR statistics...")
    ...
    print(Fore.CYAN + "Adding players to their corresponding in-memory list object...")
    QBs: list[str] = []
    RBs: list[str] = []
    WRs: list[str] = []
    TEs: list[str] = []
